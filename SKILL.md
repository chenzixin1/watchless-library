...
4. 对 conversation 路由补齐 `work/speaker-map.json`。
   对话每轮保留已确认的说话人标签，写成 `**姓名**：` 或 `**主持人**：`；身份未确认时使用 `说话人N`。入库后标签会显示为粗体加下划线。
...
- 下载完成 → 转录完成 → 章节与关键帧确认 → 笔记与字幕齐全 → 入库并保留已有课程 → 浏览器验收。
...
保持统一视觉基线：暖白纸底、深绿色强调、纯文字 `watchless / 当前文章` 面包屑。目录页展示可直接打开的文章卡片；课程页进入后直接看到具体文章标题和图文内容，宽屏左侧为视频与章节，右侧为文章，窄屏纵向阅读。禁止为某门课程添加专属样式或修改布局。

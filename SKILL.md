...
4. 对 conversation 路由补齐 `work/speaker-map.json`。
   对话每轮保留已确认的说话人标签，写成 `**姓名**：` 或 `**主持人**：`；身份未确认时使用 `说话人N`。`**主持人问**：`、`**嘉宾回答**：` 等角色加动作的标签也要单独成段，不能留作普通正文。入库后标签会显示为粗体加下划线。
5. 为每个场景生成 `work/codex-notes/scene_NNN.md`，保留关键观点、完整上下文和画面说明；在 Light-plus 中用 `**...**` 标出有原文依据的关键数字、金句、关键事实和观点，不要整段加粗或改写原话。
6. 通读完整视频的章节笔记，生成 `work/video-summary.json`。用 3–12 条简明要点概括整段视频的主要论点、重要事实和结论，避免只摘开头或把章节标题改写一遍；区分嘉宾判断、片中数字与已核实事实，广告内容要明确标注。每条列出所覆盖的章节编号，所有章节都要被覆盖。中英双语笔记需为每条提供 `zh` 和 `en`，格式参考 `site/media/figma-astra-flight-design/video-summary.json`。

无需先生成 Watchless 的最终 PDF；学习站入库只依赖 `work/` 目录。

### 语言交付要求与检查点

分别记录笔记语言与字幕语言；两者独立验收。本次用户要求“中英文字幕”时，须提供 English、中文、中文 + English 三个选项，不能只交付英文。已有会话要求持续有效，不因用户简写“go”“进度呢”而丢失。
...
- 视频总结作为独立数据存于 `work/video-summary.json`，随笔记语言切换显示；与字幕轨道分开验收。
...
处理过程中，每隔 3–5 分钟主动向用户简短汇报一次实际进度；运行环境要求更频繁更新时，遵循更短的间隔。汇报包含：已完成什么、正在处理什么、接下来做什么，以及实际遇到的阻塞。根据日志、进程退出结果和检查点报告说明进度，不编造百分比或预计时间。阶段完成、失败或需要用户介入时立即更新，不必等到定时间隔。

收到视频处理请求后，自动衔接下载、转录、章节与关键帧、笔记与字幕、入库、浏览器验收。一个检查点通过就执行下一步；失败时先在已授权范围内修复并复查。入库是任务的必需步骤，无需用户再次提醒或确认。已有课程必须保留。进度汇报不结束任务，不以完成转录或单个视频为由停止剩余工作。

长任务运行时持续等待并检查输出，同时推进不依赖它的工作；工具支持时使用可恢复的进程会话，避免一次阻塞等待超过汇报间隔。只有整个约定流程通过验收，或遇到确实需要用户输入、授权或外部条件的阻塞时，才结束当前工作。阻塞时明确说明已完成项、阻塞原因和恢复入口；不要声称结束回复后仍会自动执行没有调度机制支持的后续步骤。

对双语学习页，在开始处理时（或接续旧项目时）记录交付约定：

```bash
python3 "$SKILL_ROOT/scripts/check_delivery.py" "$PROJECT_DIR" --site "$SITE" --id "$LESSON_ID" --init --provider whisper
```

`--provider` 必须是用户选择的方案。该命令默认记录本次双语产品要求：中文主笔记、英文笔记、en/zh/bilingual 字幕、原有课程 ID。用户只要求单语时，按其意图调整 `work/delivery-requirements.json` 的语言列表，不强加双语需求。`--init` 不覆盖已有约定或目录基线。

每个阶段结束、用户询问进度、恢复会话时，重跑不带 `--init` 的检查。结果写入 `work/delivery-checkpoints.json`；缺项返回非零退出码，这是未完成信号。完整命令和浏览器记录格式见 [references/delivery-checkpoints.md](references/delivery-checkpoints.md)。

- 下载完成 → 转录完成 → 章节与关键帧确认 → 笔记、字幕与全片要点齐全 → 入库并保留已有课程 → 浏览器验收。
- 以文件和浏览器证据判断进度；启动进程、prepare 返回成功、字幕下拉框出现均不能代表整条流程完成。
- 多视频任务维护所有项目的检查点；可先将已就绪的课程入库，随后继续剩余课程。不等待所有长视频转录结束才处理已完成的短视频。
- 用户问进度时简短报告已通过/待做项，然后继续原任务。没有阻塞时，不以“尚未入库”“继续处理中”结束工作，不等待下一次催促。
- 异步任务要等待并读取退出结果；不能只启动进程后承诺会自动继续。需要用户处理的登录/授权阻塞才暂停相关步骤。
- 任一交付文件更新后重新检查；浏览器记录绑定文件指纹，旧验收不会继续有效。机器校验不代表翻译质量或画面理解已人工通过。
...
双语任务添加 `--require-bilingual-subtitles`。入库会读取 `work/delivery-requirements.json`，在写入站点前拒绝缺失语言、空字幕、未对齐的双语时间轴、不完整笔记等问题；不允许删除约定或去掉检查参数来绕过失败。
...
- 目录出现新课程且月份分组正确
- 每个章节都有标题、时间码、关键帧和正文
- 播放器提示下方显示覆盖整段视频的 bullet 要点，切换中英文笔记时要点同步切换
- 抽查正文中的关键数字、金句、关键事实和观点以 `<strong>` 呈现，原始转录保持原样；中英文笔记分别核对。访谈还要确认说话人标签以粗体加下划线显示
- 对访谈逐章检查说话人标签；尤其核对“主持人问：”“嘉宾回答：”等前缀不再作为普通文字出现。浏览器里检查完整标签（包括冒号）的计算样式：`font-weight` 至少 700，`text-decoration-line` 含 `underline`，将样本写入 `browser-check.json` 的 `speaker_label_samples` 后才算通过
- 说话人显示真实姓名或保守匿名标签
- 点击章节、笔记和原话均能跳转视频
- 播放进度与左右栏宽度刷新后保持
...
保持统一视觉基线：暖白纸底、深绿色强调、纯文字 `Watchless Lib / 当前文章` 面包屑。目录页展示可直接打开的文章卡片；课程页进入后直接看到具体文章标题和图文内容，宽屏左侧为视频与章节，右侧为文章，窄屏纵向阅读。禁止为某门课程添加专属样式或修改布局。

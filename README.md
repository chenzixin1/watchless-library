...
把 YouTube 链接或本地视频交给 AI：它会整理全片要点、章节、关键画面和图文笔记，并加入同一个知识库。<br>
点击文字就能跳回原视频；下一段视频会继续积累在目录里。
...
![课程右侧图文笔记：章节画面、重点加粗和带下划线的说话人标签](docs/images/watch-and-read.jpg)
...
如果已经选中这个 Skill，直接发链接或文件即可。Skill 会从获取视频一路处理到入库、打开本地预览，并保留已有课程；无需再提醒它执行下一步。长视频处理期间会定期报告进度。YouTube 链接需要能够访问和下载，处理你拥有或已获授权的内容。
...
仓库已包含一门可播放的约 9 分半示例课程，带 **13 个场景、视频、关键帧、中英文笔记和三种字幕选项**。目录中还有三篇文章；它们的视频文件未包含在仓库中。可以先体验阅读效果，再配置视频处理环境。
...
播放器下方先看覆盖整段视频的要点列表，再按章节阅读笔记。章节、关键画面和文字放在一起，点击段落或时间码，就能回到视频里的对应位置。上方截图展示的是右侧文章区。

### 02 · 英文视频，也能用中文理解

要点和笔记可切换 **中文 / English（原文）**；播放器可选择 **英文、中文或双语字幕**。适合先用中文理解，再对照英文原话回看。

![实际课程页面：英文原文笔记与中英双语字幕](docs/images/english-notes.jpg)

*截图来自本地 Figma 课程。新视频需要在处理时准备对应翻译与字幕，页面展示已有语言版本。*

### 03 · 下一段视频，继续放进同一个知识库

处理好的视频会加入目录，按月份整理，也可以搜索查找。已有课程会保留；同一课程重新处理则更新原记录。

![实际文章目录：按月份整理的四篇课程文章与搜索入口](docs/images/library.jpg)

**今天整理一场访谈，明天加入一节课程，慢慢积累成自己的知识库。**

每次处理都会检查转录、章节与关键帧、笔记和字幕、全片总结、入库以及浏览器中的实际呈现。访谈中的说话人标签和正文重点也会检查；发现缺项时继续修复，不把“转录完成”当成交付完成。
